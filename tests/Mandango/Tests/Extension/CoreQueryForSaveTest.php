...
class CoreQueryForSaveTest extends TestCase
{
    ...
    {
        $article = $this->mandango->create('Model\Article')
            ->setAuthor($this->mandango->create('Model\Author')->setId($id = new \MongoDB\BSON\ObjectID($this->generateObjectId()))->setIsNew(false))
        ;
    ...
    public function testDocumentReferencesOneUpdate()
    {
        $article = $this->mandango->create('Model\Article')->setDocumentData(array(
            '_id' => new \MongoDB\BSON\ObjectID($this->generateObjectId()),
            'author' => new \MongoDB\BSON\ObjectID($this->generateObjectId()),
        ));
        $article->setAuthor($this->mandango->create('Model\Author')->setId($id = new \MongoDB\BSON\ObjectID($this->generateObjectId()))->setIsNew(false));
    ...
    public function testDocumentReferencesManyInsert()
    {
        ...
        $ids = array();
        for ($i = 1; $i <= 10; $i ++) {
            $categories[] = $this->mandango->create('Model\Category')->setId($ids[] = new \MongoDB\BSON\ObjectID($this->generateObjectId()));
        }
